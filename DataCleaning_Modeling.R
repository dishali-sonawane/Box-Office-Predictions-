...
movie_data_train<-read.csv("D:/NEU Courses/Data Mining/week2/train.csv")
#movie_data_test<-read.csv("D:/NEU Courses/Data Mining/week2/test.csv")
...
comb_data <- cbind(comb_data,mt[,names(head(sort(freq,decreasing = TRUE),n=20))])
...
pranav_train<-read.csv("train_pranav.csv")
pranav_train$X<-NULL
pranav_train$X.1<-NULL
pranav_train$X.2<-NULL
#pranav_train$production_companies<-NULL

#create dummy variable for production company
dummy_prod_company<-createDummyFeatures(pranav_train$production_companies)
...
dummy_prod_company$`0`<-NULL
dummy_prod_country$`0`<-NULL

pranav_train$production_companies<-NULL
pranav_train$production_countries<-NULL

pranav_train$month<-NULL
pranav_train$weekday<-NULL

#add dummy variables to the existing dataset
updated_pranav_train<-cbind(pranav_train,dummy_prod_company,dummy_prod_country,dummy_month,dummy_weekday)
...
reduced_pranav_train<-updated_pranav_train[ , (pvalue$up_att_name)]
reduced_pranav_train<-cbind(reduced_pranav_train,x)

x<-as.data.frame(updated_pranav_train$revenue)
colnames(x)<-c("revenue")
reduced_pranav_train$`updated_pranav_train$revenue`<-NULL

reduced_model<-lm(revenue~., data = reduced_pranav_train)
